...
# Load LLM judge config from config.py
...
def _create_eval_client(judge_server='configapi'):
    """Create an OpenAI-compatible client for the eval judge model.
    Uses config.py API when judge_server='configapi', OpenRouter when 'openrouter',
    otherwise uses local vLLM.
    """
    if judge_server == 'configapi':
        return OpenAI(
            base_url=LLM_CONFIG["base_url"],
            api_key=LLM_CONFIG["api_key"],
        )
    elif judge_server == 'openrouter':
...
def evaluate(sample, model_name=None, judge_server='configapi'):
    ...
    """
    # Use config.py model name when judge_server is configapi
    if judge_server == 'configapi':
    ...
    # Only add /no_think suffix for models that support it (Qwen3 etc.)
    no_think_suffix = "" if judge_server == 'configapi' else " /no_think"
...
def start_vllm_server(model_name, tensor_parallel_size):
    ...
    # Use vllm from the same Python environment as the current process
